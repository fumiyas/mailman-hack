...
* `<FML リストディレクトリ>`
    ...
    * ***注意:***
      単一のメーリングリストディレクトリを指定してください。
      メーリングリスト(ディレクトリ)群のあるディレクトリではありません。

### 移行元データ

* `/var/spool/ml/<リスト名>` (FML メーリングリストディレクトリ): 必須
    * `aliases` (メールエイリアス)
    * `config.ph` (設定)
    * `seq` (投稿メール連番)
    ...
    * `spool/*` (保存書庫。任意)
* `/var/spool/ml/etc/aliases` (FML メールエイリアスファイル): 任意
    * FML ホストで稼働する MTA が参照しているメールエイリアスファイル。
    * 移行ツールはデフォルト値で `/var/spool/ml/<リスト名>/aliases` を参照するが、
      差分がある場合は `--fml-aliases` オプションでこのファイルを指定する必要がある。
    * ***注意:***
      通常このファイルは全 `/var/spool/ml/<リスト名>/aliases`
      を統合した内容になっているはずだが、
      稀に手動で直接書き換える運用をしている例がある。(要調査)
    * ***注意:***
      通常 MTA はこのファイル (から生成するデータベースファイル) を参照しているが、
      `/etc/aliases` など別のファイルで管理している例もある。(要調査)
    * `makefml recollect-aliases` で再作成可能。
      ***注意:***
      手動で直接書き換えている場合は変更が失なわれるので注意。
...
* Mailman は会員と非会員で別々に投稿の許可・拒否・保留・破棄が判定される。
    ...
    * FML はメール投稿許可アドレスリスト (`members` ファイル) と投稿メール配信先アドレスリスト
      ...
      このため、両用のファイルのアドレスをすべて会員として登録し、会員フラグにより投稿制限と配送有無を制御します。
        * `members`, `actives` 両方に登録されている会員:
            * 「制限会員」フラグ: なし
            * 「配送停止」フラグ: なし
        * `members` のみに登録されている会員:
            * 「制限会員」フラグ: なし
            * 「配送停止」フラグ: あり
        * `actives` のみに登録されている会員:
            * 「制限会員」フラグ: あり
            * 「配送停止」フラグ: なし
